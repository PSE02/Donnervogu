...
class ProfileId < ActiveRecord::Base
  ...
  def self.count_outdated id = nil
    self.where("time_of_last_ok <= ? AND emailaccount_type = 'listed'", (Time.now - self.threshold_for_oldest_ok)).group(:emailaccount_id).count.size # outdated email accounts, not outdated profile ids
  end
